...
void pageRankParallelStrategyTwo(Graph &g, int max_iters, const uint n_workers)
{
  // Initialization
  uintV n = g.n_;
  uintE m = g.m_;
  ...
  std::vector<thread_status> thread_status(n_workers);
  CustomBarrier barrier(n_workers);
  const uint edges_per_thread = m / n_workers;
  uint remander_edge = m % n_workers;
  uintV curr_vertex = 0;
  uint curr_edge = 0;
  ...
  t1.start();
  partition_timer.start();
  // Distribute edges to each thread
  for (int i = 0; i < n_workers; i++)
  {
    // Calculate the range of edges for the current thread
    uintV start_vertex = curr_vertex;

    // Iterate through vertices to find the end vertex for the target edge count
    while (curr_edge < (i + 1) * edges_per_thread && curr_vertex < n)
    {
      curr_edge += g.vertices_[curr_vertex].getOutDegree();
      curr_vertex++;
    }

    // Store the range of edges for the current thread
    start[i] = start_vertex;
    end[i] = curr_vertex - 1; // Subtract 1 to account for 0-based indexing
  }
  partition_time = partition_timer.stop();
...
int main(int argc, char *argv[])
{
  ...
  case 2:
    ...
    pageRankParallelStrategyTwo(g, max_iterations, n_workers);
    break;
  case 3:
